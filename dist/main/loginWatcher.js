...
exports.startLoginWatcher = startLoginWatcher;
...
async function saveOnlinePlayers(players) {
    try {
...
async function loadOnlinePlayers() {
    try {
        ...
        if (await fs.pathExists(playersPath)) {
            ...
            return new Map(playersArray.map((p) => [p.steamId, p]));
        }
    }
    catch (error) {
...
function createPanelMessage(onlinePlayers) {
...
async function startLoginWatcher(fileManager) {
    ...
    async function processAllLoginFiles() {
...
//# sourceMappingURL=loginWatcher.js.map