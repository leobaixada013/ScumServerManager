...
async function copyLatestChatFile(logsPath: string): Promise<void> {
  try {
...
async function readOffsets(): Promise<Record<string, number>> {
  try {
    ...
    if (await fs.pathExists(offsetPath)) {
      ...
      const lastOffsets = (global as any)[lastOffsetsKey];
      if (!lastOffsets || JSON.stringify(lastOffsets) !== JSON.stringify(offsets)) {
        (global as any)[lastOffsetsKey] = offsets;
      }
      return offsets;
    }
  } catch (error) {
...
function parseChatLine(line: string): { steamId: string; playerName: string; message: string; timestamp: string; type: string } | null {
  try {
...
export async function startChatGlobalWatcher(fileManager: FileManager) {
  ...
  try {
    if (await fs.pathExists(processedMessagesFile)) {
      ...
      savedMessages.forEach((messageId: string) => processedMessages.add(messageId));
  ...
  function generateMessageId(data: any): string {
  ...
  function sortChatLogsByDate(files: string[]): string[] {
    return files
      .filter(f => f.startsWith('chat_') && f.endsWith('.log'))
      .sort((a, b) => {
